...
# ---------- Banner ----------
printf '\n%s%s ROM Fastboot Flash %s\n' "$C_BOLD$C_BLUE" "▶" "$C_RESET"
printf '%srom: %s   recovery: %s (or fallback from payload)%s\n' "$C_DIM" "$ROM_ZIP" "$RECOVERY_IMG" "$C_RESET"
...
if [ -d "$IMAGES_DIR" ] && [ -n "$(ls -A "$IMAGES_DIR" 2>/dev/null)" ]; then
    ...
    if [ "${#missing[@]}" -gt 0 ]; then
        die "'$IMAGES_DIR' exists but is missing images for: ${missing[*]}. Delete '$ROM_DIR' and rerun to re-extract."
    fi
    ok "cached images verified complete"
else
    ask_yes_no "Make sure '$ROM_ZIP' is in this folder (recovery.img is optional — falls back to payload's if missing). Ready?" \
        || die "place '$ROM_ZIP' in this folder, then rerun."

    [ -f "$ROM_ZIP" ] || die "'$ROM_ZIP' not found in current directory ($(pwd))."
    ok "$ROM_ZIP found"

    info "unzipping $ROM_ZIP -> $ROM_DIR/ ..."
    unzip -o -q "$ROM_ZIP" -d "$ROM_DIR" || die "failed to unzip $ROM_ZIP."
    ok "extracted"

    [ -f "${ROM_DIR}/payload.bin" ] || die "'payload.bin' not found inside $ROM_ZIP."
...
printf '\n%s%s ROM flash complete (%s flash) — %ds total%s\n\n' \
